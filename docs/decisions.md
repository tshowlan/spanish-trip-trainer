...
- 2026-07-24 — Band tolerance ±0.2 min; sub-tolerance deviation is model noise, never a work item.
- 2026-07-24 — Kit overage sanctioned when it buys build-eligibility; re-measure with real telemetry before any trim. (First words 6.8, Numbers 6.3: kept as-is.)
- 2026-07-25 — Tom's fix batch (five rulings): (1) Practice's Recommended = the stalest LEARNED lesson (weakest seen topic fallback), never the next lesson — the action tile owns "what's next"; (2) sound boards open with a voice: the first tile auto-plays on entry (pairs + sound choice, 450ms); (3) no replay on a correct pair match — supersedes the pairs artifact's "sound, meet spelling" line (canvas delta for next re-issue); (4) the close-swap chip carries the whole target ("Ask: Where is this one?"), never the bare filler; en shells must break at word boundaries (mixed "Where is"/"Where's" families composed "Wherethis?" — glosses normalized in both packs, degenerate shells refused); (5) the speak exercise's escape always works: live recognizers abort on Cant/resolve, tap-while-live stops instead of stacking, and an 8s watchdog clears silent engine hangs.
- 2026-07-24 — Listening family designed: hero audio stage (sound is the centerpiece), played-line reveal on every listening resolution (the ear never loses the answer), wrong-then-release + settle uniformity, distractor axes split (sound vs meaning). Tom's items 5+7 closed by design.
- 2026-07-24 — Sound-off escape: whisper-tier above Continue on every listening exercise; escapes are whispers in the exit zone, persistence is system (session flag + sibling swaps + menu reset; OS-mute prompts, never silent).
- 2026-07-24 — THE FIELD: the facet landscape (73 facets, three-quarter reach, large-outside/small-inside with sprinkles and one big pane) is the exercise ground app-wide — one field, one volume (per-type registers rejected: the ground never pumps between reps). Facets never compete with words; collisions get local fixes. Over the field every functional surface is opaque, instrument tracks included.
- 2026-07-24 — THE LIGHT LAYER: light lives IN the triangles (six blazing shards, seven gradient panes), clock-bound to local daylight (7–19 [tune]) in both themes, evaluated at session start. The landscape tracks the sun, not the theme; at night the app's light rests in the instruments. No direction vector ever; static always.
- 2026-07-24 — exercise-base v2.1 ratified, derived from the artifact; base updates are re-derivations, never hand-patches. Graveyard: silhouette + bubble stage options; the register concept.
- 2026-07-25 — Code build notes on the family: the artifact's .linkbtn.whisper ships as .whisper-escape (the app's .linkbtn is an older different component — grep-the-class-name law); listening misses still open the correction sheet per §3.3 with the played-line reveal + wrong-then-release running behind its scrim (chat to rule if the sheet yields to the in-place reveal for the ear); the listen_type footer keeps "Can't tell, skip" (predates the artifact, which never saw footers) — chat to rule at next re-issue; dark-theme matched pairs cards go solid over the field (the opacity law outranks their old frost).
- 2026-07-24 — One correction surface per miss (sheet yields to in-place reveals, ear family first); per-rep skips retired from listening — escapes handle modality, misses handle difficulty.
- 2026-07-25 — Sheetless extended family-wide: earReveal ON for sound_choice (correct option carries the reveal: wash + replayable), audio_cloze (fill settle) and ear_build (assembly settle); listen_choice/listen_type keep the played-line. One correction surface per miss, restated: the in-place reveal where it exists, the sheet only where nothing else reveals.
- 2026-07-25 — The Arrival shipped as the splash: black + lighthouse (tinted by its own light), eclipse from behind the lantern, daybreak to the theme's ground, formation in session-end's meter (dials at photo completion), the candle closing (home's corner glow filling 1.8s, one curve). Tint ON, relight ON as spec.
- 2026-07-25 — Graveyard: the two-flash satellite-scan variant (world-height zoom + dotted pin — explored fully, cut for one-light simplicity; lives here if the in-flight experience ever wants a map moment); the traveling comet (whispered, then retired — the candle needs no visible courier); scale-based ignition (the line-catch); waypoint easing (pulses — curves carry character, keyframes carry structure).
- 2026-07-25 — Welcome gains 'Have an account? Sign in' under Plan my trip: login must be reachable before the intake (reinstall recovery). Canvas delta for the onboarding artifact at next touch.
- 2026-07-26 — DEVICE RENDERING family ratified (the arrival's five device catches, verbatim into the constitution): opacity-only formations for layout-transform elements, exits fill forwards only, glows are layers never filters, eased+dithered low-alpha gradients, closest-side radial sizing. Process law: anything luminous gets a device screenshot before the stamp. Precedence: the committed splash-arrival.html (five fixes encoded) is canvas truth; chat re-derives from the repo file at next touch.
...
- 2026-09-03 — Review-room structure: the scope ceiling (3 sentences / the full meal / service chit-chat); overhearing = core, three channels (ears, eyes, role-inversion); the essence guard test; the Rehearsal/SFB rationale; THE CIRCUIT brands non-scene review; scene-vs-circuit by due-mass [tune]; read-sign ships now, read-menu queued behind its design piece; writing waves 1-3 with the small-talk lesson as the priority authoring gap.
- 2026-09-06 — Profile-keyed beats (when/fill); the intake grammar field (three options, framed as grammar); neutral primary + chosen variant for gendered phrases; plain-English venue naming (Café Paloma, Rosa's, Toni's Bar).
- 2026-09-08 — Function words (en, a, al, del, de, lo, la, que, con, para, por, sin): name once, let it work — the anchor names the glue at first meeting, the tap-gloss is the on-demand job description, the weld's bare tiles are the drill. No grammar screen. "agreement" is banned from rendered copy; house phrase: "works for everyone."
...
- 2026-09-12 — Chapter one's rung: two forms, one per lesson (finish the sentence with choices in the first kit half; build the sentence from tiles in the second); the lap and close run in the lesson's form; the letter fill leaves chapter one and returns in chapter two as the word-in-sentence rung; bare-letter fills never return; contexts held to ≤6 words, one clause.

- 2026-09-13 — The beginning of the journey SHIPPED (v278): rooms, kit halves, chapter-one floor (no cold typing, replays included), flat difficulty, four rotating rungs, the anchored rung layout measured off Duolingo (16% / 63% / 47-53% / 73%), plain directions on kit rungs, Continue as the bottom bar. Tom played every increment in staging first.

- 2026-09-17 — The chapter flow (Tom): chapter 0 is the base layer, words you say, hear, or read, met in two-to-four-word contexts; chunks and welds next; sentences by place; conversations. The no-consecutive-shape rule (9/11) retires. Bar and café are one counter; the generated allergy/dietary lessons retire into Table words + the asks (pending The asks).
- 2026-09-17 — Chapter flow, chat's pass: chapter 1 laps on tiles, cold with chapter 2 (sentences by place), lap first; one chunk session of eight; chapters render 1-4 (The words · Your first sentences · Sentences by place · Conversations); the glue allowance (o, un/una, item articles, de ride by tap, never items, never under test); "already met" = the session's cards plus prior sessions; four pack contexts trimmed to the rule.
...
- 2026-09-25 — The chapter door SHIPPED (v359) and chapter one is closed (Tom). Source Serif 4 joins the type system for the door's title and line (a book serif with Jakarta's even strokes); Playfair stays in the logo only.
- 2026-09-25 — Chat's chapter-2 rulings: ¿Tiene? is the ninth ask (The asks · 2); nine room fillers earn cards, five ride the gloss, el menú → la carta; days are a chapter-1 read session; zumo and té join the counter; reply sets written; the small frames wait for chapter 3; the chapter door is the one non-grammar door (serif, lighthouse, centered; the fourth face confined to it). Glue glosses: de, lo, que rewritten, la tweaked.
