...
**Sequence is the app's; pace is the learner's.** The composer, SRS, and unlock order decide what comes next — the learner never needs curriculum control. The learner decides when: every moment's exit is a learner action (a tap, an answer, a dismissal); the app may author motion within a moment, but never spends the learner's time without giving them the tap that ends it. Control is offered, never demanded — defaults do the guiding. Scaffolds, pauses, and training wheels fade with demonstrated competence, earned by data `[tune]`, never assumed (the expertise-reversal principle).
- **Redundancy rule:** audio and displayed text must match — never narrate one phrase while a different phrase is on screen.
...
- **One light source** (decisions 2026-07-17): the active tab's under-glow **is** the app's light, and each tab's screen atmosphere is that light scaled up, blooming from the tab's x-position at the bottom edge. This retroactively rationalizes home's bottom-left ground glow (home is the leftmost tab). On tab switch **the light travels**: nav notch + glow glide (260ms), screen bloom sweeps (300ms). **Exceptions:** interstitial/ceremony surfaces (score reveal, pattern moments, future Landed-Tripfluent, session summaries) use the **top-center tint** in both themes (no nav present); **home** composes its destination photo *above* its bloom.
- **Affordance pulses are lighter than their element** (decisions 2026-07-17): a pulse annotating an element must be visibly **LIGHTER** than that element, never the element's own token (light: bright `--accent` + soft glow; dark: luminous `#ffe066` + glow). A pulse runs **only while its action is awaited**, then stops.
...
- Non-blocking notices (e.g. a transient status) are small top pills, `--t-base` fade, auto-dismiss 1.5s. **Toasts never carry learning content** (2026-07-19): anything worth reading never auto-dismisses. Transient chrome is reserved for plumbing confirmations (settings, account). Learning information lives on learning surfaces at its scale.
- **Forgiveness is silent** (2026-07-19): what we accept, we accept fully — never qualify a win. No accepted-with-asterisk patterns: no "correct but slow," no "accepted, but the canonical form is...," no slip annotations.
- **Sheet dismissal rule:** informational sheets (readiness detail, future stats/info sheets) dismiss on scrim-tap; the non-dismissible forced-beat pattern is reserved for miss pedagogy (correction sheet) and deliberate education moments. Trapping is a teaching tool, never a default.
...
- **Correct feedback = the resolution frame** (design/resolution-frame.html, 2026-07-19; supersedes the wash-only spec and design/correct-feedback.html, kept for history). ONE resolution language on all nine exercise types: every correct answer **matures the page in place** — placed tiles FUSE where they stand (tile chrome dissolves, gaps close to word spacing, 300ms), a green sweep draws under the learner's own sentence, **es always reveals** (listening included: sound meets spelling), en + full-phrase native audio materialize, the item's strength ring ticks with a "Stronger" whisper, and the unused tray recedes to 28% (nothing vanishes). The wash survives as the base layer on input-style exercises. Choreography (r5): fusion 450ms, sweep at 350ms, materialization at 600ms — a staggered sequence under the value-change duration class (it runs parallel to reading and gates nothing), sanctioned over the 300ms interaction cap. **Exit is self-paced (Pacing Rule §1.2): Continue materializes with the frame; audio autoplays; no auto-advance, no timers.** **Resolution rule: only a miss may interrupt** — sheets are miss pedagogy. **Yours-now tier:** words joining the standard materialization (green YOURS NOW kicker + "Produced cold, no help. This one travels with you."), typed/spoken production only (tiles are scaffolding — scaffolded success is never cold), once per item ever, trigger = cold axis 0→1. **Scale ladder:** each feedback surface shows the quantity that honestly moves at its timescale — rep → item strength ring; session → Readiness delta (session-end summary, penciled); week → trend on Progress. Each number appears exactly once, at the scale where it is true.
...
9b. **Luminous stamps require a device screenshot (2026-07-26):** anything with glows,
    gradients-over-dark, filters, or layered animation gets photographed on a real phone
    before the stamp — desktop review cannot see OLED banding, iOS filter clipping, or
    fill-mode timing. Chat requests it; Tom's pocket is the instrument.
