...
- **Immediate retrieval:** each new item gets a rung-1 retrieval within 2–3 slots of its presentation card. **The T₁ is served EAR-FIRST (2026-07-21):** listen_choice (audio, no es text, English meaning options). Rationale: the card hands over text+sound+meaning; read-and-pick re-tests the given channel, ear-first forces the first true decode and matches the ear-first grain. Distractors = co-introduced items (deliberate: early ear-discrimination among fresh phonology). SOUND-OFF FALLBACK: when audio is unavailable or muted, the composer serves read-and-pick meaning-pick instead — the reveal never depends on audio (existing rule, extended to selection).
...
**Riders r17–r19 (2026-07-21):** the ceremony has two clean acts — Act 1 the dials perform (0–2.85s), Act 2 the ledger reports: fact lines stagger in ONE BY ONE from ~2.95s (100ms steps, quiet pop: fade + 4px rise, 260ms each, no overshoot), Continue's promotion following the last line (~3.55s). **Continue is live from t=0** (Pacing Rule: the exit is a tap the learner HAS) as a SECONDARY outlined button that PROMOTES to the filled primary when the ledger completes — hierarchy, never opacity. An early tap calls snap-to-final (idempotent: dials at true final values, pop-ins/deltas complete, ledger fully shown) and THEN the dissolve runs — skipping loses waiting, never information. This retires the veteran-timing [tune]: expertise reversal is implemented by the user's own thumb. Reduced motion: ledger renders complete, Continue renders promoted, no stagger.
