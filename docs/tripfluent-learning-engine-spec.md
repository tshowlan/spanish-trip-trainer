...
**Active dismissal (card v2):** "Got it" is replaced by a one-tap recognition micro-rep on the card itself — e.g. "tap the part that means the check" (chunked items: tap a chunk; unchunked: tap the matching `en` among 2 soft options). Ungraded and unrecorded beyond exposure 1: a wrong tap softly highlights the correct target and the card advances on the next tap — no red, no re-queue, no counters. Rationale: attention verification + generation effect at recognition cost; the card stops being skimmable without becoming a test. The §6.1b immediate-retrieval slot (T₁) is unchanged — the micro-rep does not substitute for it. Design artifact: presentation-card v2 (revision of `design/presentation-card.html`).
...
11. **Frame-prerequisite violations (§11.1 r11):** phrases whose `frame` has no earlier machine lesson or family member in unlock order.
