...
- Gain bumps stalled (two short thin sines read quiet at any level), so the clip changed
  ...
  renderer gained peak-normalization - correct now renders at 0.95 true peak, the loudest
  possible without distortion. Win/wrong untouched (Tom ruled win about right).
...
- design/machine-family.html committed (stamped r6). Deltas live: D1 one replay-hint string
  ...
  dresses in place, a real production rep); D4 exchange miss = wrong-then-release THEN the
  correct washes (never instant auto-select; sound_choice renderer queued for the same
  pattern); D5 THE FLIGHT built exactly (real seat at true size, frozen composite - drift
  measured at -4px mid-flight, was +10 before counting seat margins - hero flies alone 420ms
  transform-only, words rise after landing, stage collapses once); D6 tiles are bare, the
  fuse dresses them (forge + weld).
- Flow ruled + composed: Forge -> Conveyor 4x1 -> Weld x2 -> Exchange x2 -> Close; machine
  encore 2 reps [tune] and the climb over conveyor+weld land in increment 5.
- Six anti-drift rules merged (constitution: Anti-drift family; README-FIRST): headlined by
  ARTIFACT-SHELL.HTML now in the chat-design-pack - canonical skeleton, fonts to weight 800
  (the old base stopped at 700: every past artifact synthesized its boldest weight), live css
  embedded, regenerated by the ritual script (design/artifact-shell-template.html +
  tools/build-artifact-shell.mjs). Four decisions recorded.
...
- Tom: the correct ping was missing on the new-phrase card. The present card's two-option meaning
  ...
  dings; the wrong tap stays soft on purpose (the card is ungraded and can't be failed - it
  gold-rings the right answer instead).
...
- Ruling landed: chains skip the guess moment. The pre-lesson flow DID object (primer was gated on guessItem) — gate now accepts scene-only primers, guess step skips without a guessItem, and chain guesses borrow decoys from the deck (verified live: dinner scene->skip, lastnight 3-option guess).
- Next: atlas re-score -> read residue together. Then Mexico (pack-state file already exported; inherits all rulings).
- File: tripfluent-spain-completion.md
...
- MACHINE EXPOSURE LEDGER built (new Machines tab): full-journey projection (every lesson first pass + replay + 6 practice sessions), counting own-lesson reps, close runs, filler credits, practice slots per machine. FIRST VERDICT: all seven machines land 20-26 total reps, none under the 60%-of-median line - the spiral holds the foundation, as designed.
- Telemetry answer for chat: NO per-slot elapsed-time logging exists yet (recordAnswer carries no timing); cheap to add when wanted [tune -> measure].

## 2026-07-22 — CONTENT ATLAS built (dev/atlas.html): the big-picture audit instrument
- Tom's ask: see how everything ladders up + find the gaps. A dev tool (harness-class, localhost-only, read-only sandbox — save() neutered, sims run on in-memory state) computing EVERYTHING from the live engine: real composeSession runs (12 per lesson per scenario), never spec descriptions.
- MAP: every lesson (both packs) with phrase count, primer status, machine/gated badges, est. time vs the 6-min target (4-8 band), difficulty 0-10, gap flags (thin/short/long/chunkless/keywordless/guess-stale/no-primer). Practice session card included. LESSON DETAIL: primer + one real composed run as a slot strip (ear-first T1s outlined, the close marked) + aggregate type mix + the difficulty ramp curve + per-item eligible-modes table. ENGINE FORKS: the ladder + mode families rendered from live constants, plus 13 fork/deviation cards (purity, T1 fork, first-pass cap + close exemption, second chance, rung-down, variety, replay, feasibility fallbacks, principled distractors, forgiveness, cram, escalation, session end) each with code anchors. AUDIT: sortable table, pass-median difficulty deviation, band outliers.
- Models are the audit's dials, all [tune] and shown in-tool: seconds/slot, difficulty/type + item modifier, 6-min target. Clean-run estimates (misses add time).
- FIRST REAL FINDINGS on load: Spain first-pass — 24 open gaps, 16 of 35 lessons outside the 4-8 min band (all 7 machines ~2.5 min short; Numbers that pay 7.0 min at difficulty 1.4 = long AND flat); Mexico: large unprimed set (primer wave hasn't arrived). The instrument works.

## 2026-07-22 — Home action stack SHIPPED (v176) — harness passed; four rulings applied (duration-promise law, presence visible, type tiers, scales pencil)
...
## v307 (2026-09-22) — staged (enter-bloom): the tile itself grows to fill the screen (from Tom's Duolingo recording)
- Tom recorded Duolingo's entry and the first cut had it wrong: there the CARD grows to fill the screen; the light is tight around its edge and grows with it. Frames read off the recording: breathe up ~8% with an edge glow (~170ms) → the card expands to the whole screen while the page fades (~100ms) → a dark hold (~150ms) → the exercise fades in (~300ms). `enterWith()` is rebuilt on that: a cover the exact size and corner radius of the tapped tile, in the tile's surface color, breathes to 1.06 with a tight gold edge, then its box animates to the full viewport with the glow spreading (160ms), the page and any open sheet fade to 12%, the next screen paints under the cover at 380ms, the cover fades over 300ms. Same three doors (Home tile, Practice option, Learn row). Measured at 375×812: cover 327→375 wide and 135→812 tall between 170 and 420ms; runner on screen at 420ms; cover gone by 900ms.
...
- ENGINE: `_chapterFlowDeck()` (engine.js) arranges it when the switch is on: stage 0 = The words; stage 1 = Your first sentences (rooms in the order Asking 1, Finding 1, Finding 2, Asking 2; the generated allergy/dietary lessons sit after them until The asks carries "Tengo alergia a"); stage 2 = Sentences by place (today's chapters 1 + 2, in order); stage 3 = Conversations. A word that a later lesson also carries is ONE item object (one id, one history; counted once in ALL_ITEMS). Old-kit phrases that are not words re-home (¿Habla inglés? and the two polite greetings to I only speak a little; the two number phrases to Euros & paying cash; the emergencies to If something's wrong; Pase, pase and ¿Está todo bien? to What locals say to you). Nothing is lost.
- FLOOR (chat's ruling): first sentences stay on tiles, lap included; cold enters with Sentences by place, lap first; conversations run the full ladder. Reviews, scenes and circuits keep the full ladder.
- LEARN TAB: kickers read CHAPTER 1-4 with the new titles when the flow is on; word sessions count "words".
- Kit split rule (ruling 4) applies: Counter, Street and Signs run as halves (Table too when the profile adds words), so chapter 1 is 14-15 sessions.
- Verified: staging off = the shipped v278 deck exactly; staging on = all word contexts buildable by the four rungs, no duplicate ids, no lost phrases; Paying words plays cards, words board, rotating rungs, listening lap.
- Code's two content edits, flagged to chat: "dentro / fuera" split into two items (dentro: "¿Dentro o fuera?"; fuera: "Fuera, por favor.") so the word under test is not answered inside its own sentence; four allergen words chat's table did not list (cacahuetes, huevo, pescado, soja) written to chat's "Sin ___, por favor." pattern.
- NOT in this increment: The asks (chunk session + its two rungs), the HEAR and READ emphases, primers and beats for the new sessions, retiring the generated allergy/dietary lessons, the order inside Sentences by place.
